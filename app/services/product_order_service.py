...
class OrderProcessor():
    @staticmethod
    async def update_stock_count(order_data: ProductOrder):
        try:
            if len(order_data.items) > 0:
                ...
                if item.quantity > 0:
                    ...
                    if stock_data["stock_count"].values[0] < item.quantity:
                        return (False, item.product_id, "Quantity exceeded for Product {} ({})".format(product["product_id"], product["name"]))
                    ...
                    return (True, item.product_id, "Product {} ({}) was purchased".format(product["product_id"], product["name"]))
